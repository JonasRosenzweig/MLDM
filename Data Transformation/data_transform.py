...
unused_headers = ['PROD_PDF_URL_TEXT_3', 'PROD_PDF_URL_TEXT_2', 'PROD_PDF_URL_TEXT', 'PROD_HIDDEN_PERIOD_ID',
                  ...
                  'PROD_LOCATION_NUMBER', 'DESC_LONG_2', 'DESC_SHORT', 'PROD_SEARCHWORD', 'PROD_CREATED_BY', 'PROD_EDITED_BY']
removed_headers = ['PROD_HIDDEN', 'PROD_VAR_MASTER', 'PROD_NEW', 'PROD_FRONT_PAGE', 'TOPLIST_HIDDEN',
                   'OMIT_FROM_FREE_SHIPPING_LIMIT',
                   'PROD_CREATED', 'PROD_EDITED',
                   'LANGUAGE_ID', 'PROD_WEIGHT', 'PROD_PHOTO_URL', 'CURRENCY_CODE', 'DIRECT_LINK',
                   'PROD_SHOW_ON_GOOGLE_FEED', 'PROD_BARCODE_NUMBER', 'PROD_SHOW_ON_FACEBOOK_FEED',
                   'PROD_UNIQUE_URL_NAME',
                   'PROD_SHOW_ON_PRICERUNNER_FEED', 'PROD_SHOW_ON_KELKOO_FEED', 'PROD_RETAIL_PRICE', 'PROD_PDF_URL',
                   'PROD_PDF_URL_2',
                   'PROD_PDF_URL_3', 'PROD_DELIVERY', 'PROD_DELIVERY_NOT_IN_STOCK', 'FIELD_1', 'FIELD_2', 'FIELD_3',
                   'FIELD_4', 'FIELD_5',
                   'FIELD_6', 'FIELD_7', 'FIELD_8', 'FIELD_9', 'FIELD_10', 'PROD_FILE_URL',
                   'ïï\x06Óµ\x07L¸Úb.j"ÿ"%\x165´', 'EDBPriser_NUM',
                   'PROD_SORT', 'PROD_SITE_SPECIFIC_SORTING', 'PROD_UNIT_ID', 'PROD_NUM']
kept_headers = ['PROD_NAME', 'TITLE', 'DESC_LONG', 'META_DESCRIPTION', 'VENDOR_NUM', 'INTERNAL_ID', 'PROD_TYPE_ID',
                'MANUFAC_ID', 'PROD_CAT_ID', 'PROD_RETAIl_PRICE', 'PROD_COST', 'UNIT_PRICE', 'PROD_COST_PRICE']
count = ['PROD_SALES_COUNT', 'STOCK_COUNT', 'STOCK_LIMIT', 'PROD_VIEWED', 'PROD_MIN_BUY', 'PROD_MAX_BUY',
         'PROD_MIN_BUY_B2B']
price = ['PROD_RETAIl_PRICE', 'PROD_COST', 'UNIT_PRICE', 'PROD_COST_PRICE',]
description = ['DESC_LONG', 'META_DESCRIPTION', 'DESC_LONG_2']
ID = ['VENDOR_NUM', 'INTERNAL_ID', 'PROD_TYPE_ID', 'MANUFAC_ID', 'PROD_CAT_ID']
...
for n in range(len(price)):
    df = df.replace(price[n], 'PRICE')
for i in range(len(description)):
    df = df.replace(description[i], 'DESCRIPTION')
for j in range(len(ID)):
    df = df.replace(ID[j], 'ID')
for k in range(len(title)):
    df = df.replace(title[k], 'PROD_TITLE')
df = df.head(100)
print('Removed unwanted headers.')
df.to_csv('MVP_train_PRICE_DESC_ID_TIT_100.csv', index=False)
